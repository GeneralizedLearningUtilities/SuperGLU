...
var gatewayMapping = {
    "SocketIOGateway": "edu.usc.ict.superglu.core.SocketIOGateway",
    "ActiveMQTopicMessagingGateway": "edu.usc.ict.superglu.core.ActiveMQTopicMessagingGateway",
    "GIFTVHumanBridge": "edu.usc.ict.superglu.vhuman.GIFTVHumanBridge",
    "RestGateway": "edu.usc.ict.superglu.core.RestGateway",
    "PostService": "edu.usc.ict.superglu.core.PostService"
};

//to display service and gateway as different colored nodes
var typeMapping = {
    "SocketIOGateway": "gateway",
    "ActiveMQTopicMessagingGateway": "gateway",
    "RestGateway": "gatewat",
    "GIFTVHumanBridge": "service",
    "PostService": "service",
    "Others": "service"
}
...
$(function () {
    "use strict"
    //if query param named file exists, get the file from the url and load it.
    var url = getParameterByName('file');
    if (url) {
        $.get(url, function (data) {
            uponClick(data);  //draw graph once JSON is set
        });
    }
});
...
function drawgraph(s, jsonObj) {
    ...
    var weights = {
        'edu.usc.ict.superglu.core.ActiveMQTopicMessagingGateway': 1,
        'edu.usc.ict.superglu.core.SocketIOGateway': 2,
        'edu.usc.ict.superglu.core.RestGateway': 3,
        'edu.usc.ict.superglu.vhuman.GIFTVHumanBridge': 4,
        'edu.usc.ict.superglu.core.PostService': 5,
        'Others': 6
    };
