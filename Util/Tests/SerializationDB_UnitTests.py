...
class TestClassSimple(Serializable):
...
class TestClassComplex(Serializable):
...
class Serialization_Tests(unittest.TestCase):    
    ...
    def testDecorated(self):
        ...
        class TestDecSimple(Serializable):
            def __init__(self, value=None):
...
@DBSerialized()
class IndexedSimple(Serializable):
...
@DBSerialized()
class IndexedComplex(Serializable):
    @classmethod
    def INDEXES(cls):
